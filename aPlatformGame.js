...
class Level {
	constructor(plan) {
		...
		// top left being (x=0,y=0)
		this.rows = rows.map((row, y) => {
			return row.map((ch, x) => {
			let type = levelChars[ch];
			if (typeof type == "string") return type;
			this.startActors.push(type.create(new Vec(x, y), ch)); //either Player, Coin or Lava // add Monster
			return "empty";
			});
		});
	}
...
function runAnimation(frameFunc) {
  let lastTime = null;
  function frame(time) { // the callback function is passed a single argument: DOMHighResTimeStamp
    if (lastTime != null) {
...
function runLevel(level, Display) {
  ...
  return new Promise(resolve => {
    ...
    const arrowKeys = trackKeys(["ArrowLeft", "ArrowRight", "ArrowUp"]); // add Event Listeners
    

    function frame(time) {
      if (running == "pausing") {
        running = "no"
        return false; // the animation is actually stopped
      }
